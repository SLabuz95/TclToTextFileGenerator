#include"FRI_FSD_Custom_CANoeDBC.hpp"
...
bool FSD_CANoe_CANoeConfig::processingFunction<FSD_CANoe_CANoeConfig::Stat::VDATABASE_NETWORK_NAME_READ>(){
    ...
    if(interpreterData->tempPathHelper.isEmpty())
        return config.ERROR(Error::ERROR);
    static_cast<CANoeNetworksDatabaseMapping*>(interpreterData->dmStats.last())->dataBaseNetworkMap.insert(interpreterData->lastVDataBase, interpreterData->tempPathHelper);
...
bool FSD_CANoe_CANoeConfig::Config::FSFunction<FSD_CANoe::FileSpecificInterpreterStat::BEGIN_OF_OBJECT>(){
    if(interpreterData->ignoreTil.isEmpty()){
        ...
        if((interpreterData->readControl != InterpreterData::ManualControlStat::AUTOMATIC && interpreterData->readControl_setErrorIfBeginOrEndOfObject) || beginOfObjectParams.size() != 3 || interpreterData->objectMappingTrace.size() + 1 != beginOfObjectParams.at(1).toInt(&numbOk) || !numbOk || beginOfObjectParams.at(0).isEmpty())
            return ERROR(Error::ERROR);
