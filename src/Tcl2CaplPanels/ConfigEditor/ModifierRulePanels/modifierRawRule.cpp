#include"modifierRawRule.hpp"
#include"Tcl2CaplPanels/ConfigEditor/rulesphasepanel.hpp"
#include<QApplication>

using namespace Panels::Configuration::View::ModifierRules;
using ListItemType = RulesPhasePanel::ListItem;
...
{    
    ...
    if(rule != nullptr){   // Exists
        ...
        // ControlFlag
        ruleControlComboBox.setCurrentIndex(static_cast<std::underlying_type_t<ModifierControlFlag>>(rawRule.controlFlag()));
        // Conditionals
...
void RawRuleView::readRule(RuleRef rule){
    ...
    using RawRuleRef = RawRule&;
    RawRuleRef rawRule = *static_cast<RawRule*>(&rule);
    rawRule.setControlFlag(static_cast<ModifierControlFlag>(ruleControlComboBox.currentIndex()));
    // Conditionals
