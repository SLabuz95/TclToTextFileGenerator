...
ItemView::ItemView(List& list, FormatRulePtr rule)
{
    ...
    titleComboBox.addItems(ModifierFormatParameters::TypeInfo::typeNames());
    if(rule)
...
void ItemView::readRule(FormatRulePtr& rule){
    rule = ModifierFormatParametersFactory::create(FormatRule::fromUnderlying(titleComboBox.currentIndex()));
